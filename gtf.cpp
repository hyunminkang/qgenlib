...
gtf::gtf(const char *filename, std::vector<std::string> *pGenetype, bool addChrPrefix, bool removeChrPrefix, bool createGeneTranscript) : maxGeneLength(0), maxTranscriptLength(0), maxExonLength(0),
                                                                                                                                          ...
                                                                                                                                          maxStopCodonLength(0)
{
  ...
  for (line = 0; (nfields = tr.read_line()) > 0; ++line)
  {
    ...
    */
    ...
    std::string gid, tid, eid, eno, name, type;
    if (createGeneTranscript)
    { // create gene and transcript first before inserting the element. Caveat is that beg/end is misleading.
      // notice("foo");
      gid = gep.get("gene_id");
  ...
  if ( strncmp(&attr[attrFields[i]], "gene_id \"", 9) == 0 ) {
    ...
    type.assign(&attr[attrFields[i]+11], attrFields[i+1]-attrFields[i]-17);
      ...
      */
      ...
      if (git == gid2Gene.end())
      { // need to create gene. note that start and end is misleading (based on first appearing exon)
      ...
      { // need to create transcript. note that start and end is misleading
                  ...
                  std::string &gid, std::string &gname, std::string &gtype)
{
  ...
  else if (strand[0] == '-')
  {
    ...
    error("[E:%s:%d:%s] Unknown strand info %s", __FILE__, __LINE__, __PRETTY_FUNCTION__, strand);
...
int32_t gtf::findOverlappingElements(const char *seqname, int32_t start, int32_t end, bool fwdStrand, std::set<gtfElement *> &results)
{
  ...
  for (int32_t i = 0; i < (int32_t)overlaps.size(); ++i)
  {
    ...
    while (root->parent != NULL)
      root = root->parent;
    gtfGene *rootGene = (gtfGene *)root;
    if (rootGene->fwdStrand == fwdStrand)
    { // check strand and insert only consistent ones
      results.insert(iv.value);
    }
    // notice("Query = %s:%d-%d, Found %s:%d-%d, Type = %s, Gene ID = %s, Gene Name = %s, Gene Type = %s", seqname, start, end, seqname, iv.start, iv.stop, e->type.c_str(), rootGene->geneId.c_str(), rootGene->geneName.c_str(), rootGene->geneType.c_str());
  }
